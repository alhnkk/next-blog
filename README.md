...
## Route Protecting (Rota Koruma)

Better Auth kullanarak uygulamanızda güvenli rota koruması sağlamak için çeşitli yöntemler mevcuttur. Aşağıda kapsamlı route protecting stratejileri ve örnekleri bulabilirsiniz.

### 1. Middleware ile Rota Koruma

Middleware, belirli rotalara erişimi kontrol etmek için kullanılan ara katmandır. Next.js'te middleware kullanarak hızlı oturum kontrolü yapabilirsiniz.
...
export async function middleware(request: NextRequest) {
    ...
    // Çerez kontrolü (hızlı kontrol, güvenlik için yeterli değil)
    if (!sessionCookie) {
...
export const config = {
    matcher: [
        "/admin/:path*",    // Admin rotaları
        "/dashboard/:path*", // Dashboard rotaları
        "/profile/:path*"    // Profil rotaları
    ]
};
```

**⚠️ Önemli:** `getSessionCookie` sadece çerezin varlığını kontrol eder, geçerliliğini doğrulamaz. Tam güvenlik için her sayfada sunucu tarafı oturum doğrulaması yapılmalıdır.

### 2. Sunucu Tarafında Oturum Doğrulama

Her korumalı sayfada `auth.api.getSession` kullanarak tam güvenlik sağlayın.

#### Server Component ile Koruma
...
### 3. Rol Tabanlı Rota Koruma

Kullanıcı rollerine göre erişim kontrolü sağlayın.

#### Rol Kontrolü ile Middleware
...
export async function middleware(request: NextRequest) {
    ...
    // Moderatör rotaları için rol kontrolü
...
React component'lerinde oturum durumunu kontrol etmek için hook'lar kullanın.

#### useSession Hook Kullanımı
...
### 5. Higher-Order Component (HOC) ile Koruma

Sayfa component'lerini sarmak için HOC kullanın.
...
API endpoint'lerini korumak için middleware kullanın.
...
Daha granüler erişim kontrolü için permission sistemi kullanın.
...
export const rolePermissions = {
    ...
    moderator: [permissions.USERS_READ, permissions.POSTS_READ, permissions.POSTS_WRITE],
    user: [permissions.POSTS_READ]
};
...
export default async function UsersPage() {
    ...
    if (!session || !hasPermission(session.user.role, permissions.USERS_READ)) {
        redirect("/unauthorized");
    }
...
### 8. Route Guards ile Merkezi Koruma
...
### 9. Hooks ile Dinamik Koruma

Belirli eylemlerden önce/sonra özel kontroller yapın.
...
export const auth = betterAuth({
    hooks: {
        before: createAuthMiddleware(async (ctx) => {
            // Kayıt sırasında e-posta domain kontrolü
            if (ctx.path === "/sign-up/email") {
                if (!ctx.body?.email.endsWith("@company.com")) {
                    throw new APIError("BAD_REQUEST", {
                        message: "Sadece şirket e-posta adresleri kabul edilir",
                    });
                }
            }

            // Admin işlemleri için IP kontrolü
            if (ctx.path.startsWith("/admin/")) {
                const allowedIPs = ["192.168.1.100", "10.0.0.50"];
                const clientIP = ctx.headers?.["x-forwarded-for"] || ctx.headers?.["x-real-ip"];
                
                if (!allowedIPs.includes(clientIP as string)) {
                    throw new APIError("FORBIDDEN", {
                        message: "Bu IP adresinden admin erişimi izin verilmiyor",
                    });
                }
            }
        }),
...
Bu doküman, Better Auth ile kapsamlı route protection stratejileri sunmaktadır:

1. **Middleware**: Hızlı çerez kontrolü ve yönlendirme
2. **Server-Side**: Tam güvenlik için oturum doğrulama
3. **Role-Based**: Kullanıcı rollerine göre erişim kontrolü
4. **Client-Side**: React hook'ları ile dinamik koruma
5. **HOC**: Component sarmalama ile koruma
6. **API Protection**: Backend endpoint'leri koruma
7. **Permission-Based**: Granüler erişim kontrolü
8. **Route Guards**: Merkezi koruma fonksiyonları
9. **Dynamic Hooks**: Özel kontrol mantığı
10. **Security**: CSRF, rate limiting ve diğer güvenlik önlemleri

Her yöntem, uygulamanızın ihtiyaçlarına göre tek başına veya birlikte kullanılabilir.
# plugins: Admin
